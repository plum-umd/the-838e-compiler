...
#|
  ...
  (check-equal? (run '(<= 0 -1)) (<= 0 -1))
  (check-equal? (run '(<= 0 1)) (<= 0 1))
  (check-equal? (run '(<= 0 -10000000000000000000)) (<= 0 -10000000000000000000))
  (check-equal? (run '(<= 0 10000000000000000000)) (<= 0 10000000000000000000))
  (check-equal? (run '(<= -10000000000000000000 0)) (<= -10000000000000000000 0))
  (check-equal? (run '(<= 10000000000000000000 0)) (<= 10000000000000000000 0))
  (check-equal? (run '(<= -10000000000000000000 -1000000000000000000)) (<= -10000000000000000000 -1000000000000000000))
  (check-equal? (run '(<= 10000000000000000000 -10000000000000000000)) (<= 10000000000000000000 -10000000000000000000))

  (check-equal? (run '(+ 0 0)) (+ 0 0))
  (check-equal? (run '(+ 576460752303423487 2)) (+ 576460752303423487 2))
  (check-equal? (run '(+ -576460752303423487 -2)) (+ -576460752303423487 -2))
  (check-equal? (run '(+ -576460752303423487 -2)) (+ -576460752303423487 -2))
  (check-equal? (run '(+ -576460752303423490 576460752303423490)) (+ -576460752303423490 576460752303423490))
  (check-equal? (run '(+ -576460752303423490 10000000000000000000000000)) (+ -576460752303423490 10000000000000000000000000))
  (check-equal? (run '(+ 576460752303423490 10000000000000000000000000)) (+ 576460752303423490 10000000000000000000000000))

  (check-equal? (run '(- 1 3)) (- 1 3))
  (check-equal? (run '(- 576460752303423489 2)) (- 576460752303423489 2))
  (check-equal? (run '(- -576460752303423487 2)) (- -576460752303423487 2))
  (check-equal? (run '(- -576460752303423489 -2)) (- -576460752303423489 -2))
  (check-equal? (run '(- 576460752303423490 576460752303423490)) (- 576460752303423490 576460752303423490))
  (check-equal? (run '(- -576460752303423490 10000000000000000000000000)) (- -576460752303423490 10000000000000000000000000))
  ...
  (define (run-with-file rkt file-contents)
    ...
    (display file-contents file-port)
    (close-output-port file-port)
    (define result (run (rkt (path->string file-path))))
    (delete-file file-path)
    result)

  (check-equal? (run '(port? 0)) #f)
  (check-equal? (run-with-file (λ (file) `(port? (open-input-file ,file))) "") #t)
  (check-equal? (run-with-file (λ (file) `(close-input-port (open-input-file ,file))) "") (void))
  (check-equal? (run-with-file (λ (file) `(read-byte (open-input-file ,file))) "") eof)
  (check-equal? (run-with-file (λ (file) `(peek-byte (open-input-file ,file))) "") eof)
  (check-equal? (run-with-file (λ (file) `(read-byte (open-input-file ,file))) "a") 97)
  (check-equal? (run-with-file (λ (file) `(peek-byte (open-input-file ,file))) "a") 97)
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
                                     (cons (peek-byte port) (read-byte port)))) "a")
                (cons 97 97))
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
                                     (cons (read-byte port) (peek-byte port)))) "a")
                (cons 97 eof))
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
                                     (cons (read-byte port) (read-byte port)))) "ab")
                (cons 97 98))
  (check-equal? (run-with-file (λ (file)
                                  `(begin (define (read-all-bytes p)
                                            (let ((b (read-byte p)))
                                              (if (eof-object? b)
                                                '()
                                                (cons b (read-all-bytes p)))))
                                          (let ((p (open-input-file ,file)))
                                            (read-all-bytes p)))) (make-string 100 #\R))
                (make-list 100 82))

  (check-equal? (run (let ((file-dne (make-temporary-file "dne~a")))
                  (delete-file file-dne)
                  `(open-input-file ,(path->string file-dne)))) 'err)

  (check-equal? (run '(close-input-port 0)) 'err)
  (check-equal? (run '(read-byte 0)) 'err)
  (check-equal? (run '(peek-byte 0)) 'err)
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
                                     (begin (close-input-port port) (read-byte port)))) "")
                'err)
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
                                     (begin (close-input-port port) (peek-byte port)))) "")
                'err)
  (check-equal? (run-with-file (λ (file)
                                  `(let ((port (open-input-file ,file)))
