...
;; (Listof Bindings) Expr CEnv -> Asm
(define (compile-let bs e c)
  (let ((split-bs (split-bindings bs))) 
    (seq (compile-let-defs (cdr split-bs) c)
         (compile-e e (append (reverse (car split-bs)) c))
         (Add rsp (* 8 (length bs))))))
  ;;; (seq (compile-e e1 c)
  ;;;      (Push rax)
...
;; (Listof Expr) CEnv -> Asm
;; Compile the definitions of bindings
(define (compile-let-defs es c)
  (match es
    ['() (seq)]
    [(cons e es)
      (seq  (compile-e e c)
            (Push rax)
            (compile-let-defs es (cons #f c)))]))
