...
vl_val sdl_init(uint64_t argc, vl_val *argv)
{
  ...
  vl_check_type(argv[1], VL_FLONUM);
  vl_check_type(argv[2], VL_FLONUM);

  width = vl_unwrap_flonum(argv[0]);
  height = vl_unwrap_flonum(argv[1]);
  scale = vl_unwrap_flonum(argv[2]);
 ...
 *  - void
 */
vl_val sdl_pull_events(uint64_t argc, vl_val *argv)
{
