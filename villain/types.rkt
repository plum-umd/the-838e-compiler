  ...
  (cond [(= type-int (bitwise-and b mask-int))
        ...
        [(= type-float (bitwise-and b mask-float))
         ...
         (let (
               ...
               ; the max decimal places to round to are the first non-zero bits
               (sig (arithmetic-shift b (- (+ float-shift 32))))

                ; then is bit representing the sign (0 if positive, 1 if negative)
               (sign (bitwise-and (arithmetic-shift b (-(+ float-shift 31))) 1))

               ; the next 8 bits are the exponential + 127
               (exp (bitwise-and (- (arithmetic-shift 1 8) 1) (arithmetic-shift b (-(+ float-shift 23)))))

               ;; the next 28 bits represent the mantissa
               (mantissa (bitwise-and (- (arithmetic-shift 1 23) 1) (arithmetic-shift b (- float-shift))))

               )

           ;; result is a rounded version of (-1)^sign * 2^(exp - 127) * (1 + .mantissa)
           (let ((result ( * (expt -1 sign) (expt 2 (- exp 127)) (+ 1 (*  (binary->decimal mantissa 0 -23))))))
                  (round_dec result sig)))
...
(define (float->bits f)
  ...
  ;; sign = 0 if float is positve and 1 if negative
  (let ([sign (if (> f 0) 0 1)]
        ;; gets maximum decimal place to round to
        [sig (decimal_place f 0 (- 7 (integer-size (truncate f) 0)) )]

        ;; gets the exponent and the mantissa in decimal form
        [data (if (= f 0) (cons 0 0) (if (>= (abs f)  1) (calc-exp (abs f) 0 1 /) (calc-exp (abs f) 0 -1 *)))])
    (
    match data
...
(define (decimal_place dec acc up_to)
  (
   if (= acc up_to)
    up_to
    (if (integer? dec) acc (decimal_place (* dec 10) (+ acc 1) up_to ))
    )
  )
...
(define (float->bits_helper sign exp mantissa sig)
 
 (+ (arithmetic-shift (+ (arithmetic-shift (+ (arithmetic-shift (+ (arithmetic-shift sig 1) sign) 8) exp) 23) mantissa) float-shift) type-float)
 )
...
(define (calc-exp num test-exp sign op)
    ...
    (let ([check (op num (arithmetic-shift 1 test-exp))]) (if (and (>= check 1)
                                                        (< check 2))
...
(define (dec->binary dec acc twoExp)
  ...
  [_  (let ((check (* dec 2))) (if (>= check  1)
   (dec->binary (- check 1) (+ (arithmetic-shift acc 1) 1 ) (- twoExp 1))
                (dec->binary check (arithmetic-shift acc 1) (- twoExp 1))))]
...
(define (binary->decimal bits acc twoExp) (
  ...
  [_  (if (= 1 (bitwise-and bits 1))
   (binary->decimal (arithmetic-shift bits -1) (+ (expt 2 twoExp) acc) (+ twoExp 1))
                (binary->decimal (arithmetic-shift bits -1) acc (+ twoExp 1)))]
    )
)
